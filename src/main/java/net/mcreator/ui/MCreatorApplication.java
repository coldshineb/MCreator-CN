...
public final class MCreatorApplication {
	...
	private MCreatorApplication(List<String> launchArguments) {
		...
		splashScreen.setProgress(5, "加载 UI 核心");
		...
		splashScreen.setProgress(20, "预加载资源");

		UIRES.preloadImages();
		TiledImageCache.loadAndTileImages();

		splashScreen.setProgress(30, "加载插件");

		PluginLoader.initInstance();

		splashScreen.setProgress(40, "加载接口组合");
		...
		splashScreen.setProgress(45, "加载插件数据");
		...
		splashScreen.setProgress(55, "加载生成器");
		...
		splashScreen.setProgress(88, "初始化用户会话");

		deviceInfo = new DeviceInfo();
		analytics = new Analytics(deviceInfo);

		splashScreen.setProgress(100, "加载 MCreator 窗口");
