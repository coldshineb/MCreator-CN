...
public class JavaMemeberNameValidator implements Validator {
	...
	@Override public ValidationResult validate() {
		...
		if (text == null || text.length() == 0)
			return new Validator.ValidationResult(Validator.ValidationResultType.ERROR, "名称不能为空");
		...
		if (!JavaConventions.isValidJavaIdentifier(textField.getText())) {
			return new Validator.ValidationResult(Validator.ValidationResultType.ERROR, "无效名称");
