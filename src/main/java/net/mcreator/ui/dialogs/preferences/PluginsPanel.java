...
class PluginsPanel {
	...
	PluginsPanel(PreferencesDialog preferencesDialog) {
		preferencesDialog.model.addElement("管理插件");
		...
		preferencesDialog.preferences.add(sectionPanel, "管理插件");
	}
	...
	static class PluginsListCellRenderer extends JLabel implements ListCellRenderer<Plugin> {
		...
		public Component getListCellRendererComponent(JList<? extends Plugin> list, Plugin value, int index,
			...
			if ((value.getInfo().getAuthor() != null) && (value.getInfo().getName() != null) && (value.getInfo()
				...
				setText("<html>" + value.getInfo().getName() + "<br><small>ID: " + value.getID() + ", 版本: " + value
						.getPluginVersion() + ", 作者: " + value.getInfo().getAuthor() + ", 载入: " + (value
						.isLoaded() ? "<html><font color=#a7ed1a>是</font>" : "<html><font color=#f24122>否</font>"));
			} else if (value.getInfo().getAuthor() != null)
				setText("<html>" + value.getInfo().getName() + "<br><small>ID: " + value.getID() + ", 版本: " + value
						.getPluginVersion() + ", 作者: " + value.getInfo().getAuthor() + ", 许可: " + value
						.getInfo().getCredits() + ", 载入: " + (value.isLoaded() ?
						"<html><font color=#a7ed1a>是</font>" :
						"<html><font color=#f24122>否</font>"));
			else
				setText("<html>" + value.getInfo().getName() + "<br><small>ID: " + value.getID() + ", 版本: " + value
						.getPluginVersion() + ", 载入: " + (value.isLoaded() ?
						"<html><font color=#a7ed1a>是</font>" :
						"<html><font color=#f24122>否</font>"));
